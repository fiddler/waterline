        ...
        async.reduce(_.keys(joinsByAlias), parentResults, function(populatedParentRecords, alias, nextSetOfJoins) {
          ...
          if (aliasJoins.length === 2) {
            ...
            junctionTableAdapter.find(junctionTableDatastoreName, junctionTableQuery, function(err, junctionTableResults) {
              ...
              // Instead of querying per parent PK (N+1 problem), collect all unique
              // child PKs from the junction table and do a single batched query.

              // Get ALL unique child PKs from the entire junction table result set.
              var allChildPks = _.uniq(_.pluck(junctionTableResults, secondJoin.parentKey));

              // If there are no child PKs, skip the query.
              if (allChildPks.length === 0) {
                // Attach empty arrays for all parents.
              ...
              // Build a single batched query for all child records at once.
              var childTableQuery = _.cloneDeep(baseChildTableQuery);
              var childInConjunct = {};
              childInConjunct[secondJoin.childKey] = { in: allChildPks };
              childTableQuery.criteria.where.and.push(childInConjunct);

              // Remove skip/limit from the batched query since we need ALL matching children
              // first, then we'll apply per-parent skip/limit/sort in JS.
              var userSkip = childTableQuery.criteria.skip || 0;
              var userLimit = childTableQuery.criteria.limit || (Number.MAX_SAFE_INTEGER||9007199254740991);
              var userSort = childTableQuery.criteria.sort || [];
              childTableQuery.criteria.skip = 0;
              childTableQuery.criteria.limit = Number.MAX_SAFE_INTEGER||9007199254740991;

              // Run the single batched query.
              childTableAdapter.find(childTableDatastoreName, childTableQuery, function(err, allChildResults) {
                if (err) {
                ...
                // Build a lookup dictionary: child PK => child record.
                var childRecordsByPk = _.indexBy(allChildResults, secondJoin.childKey);

                // Group junction table records by parent PK in a single pass (O(M))
                // instead of filtering per parent (O(N*M)).
                var junctionRecordsByParentPk = {};
                _.each(junctionTableResults, function(record) {
                  var parentPk = record[firstJoin.childKey];
                  if (!junctionRecordsByParentPk[parentPk]) {
                    junctionRecordsByParentPk[parentPk] = [];
                  }
                  junctionRecordsByParentPk[parentPk].push(record);
                });

                // Get the unique parent primary keys from the junction table result.
                var parentPks = _.keys(junctionRecordsByParentPk);

                // Build the childRecordsByParent dictionary by mapping junction table
                // relationships to actual child records.
                var childRecordsByParent = {};
                _.each(parentPks, function(parentPk) {
                  // Get junction records for this parent via O(1) lookup.
                  var junctionRecordsForParent = junctionRecordsByParentPk[parentPk] || [];

                  // Map junction records to actual child records.
                  var childRecords = [];
                  _.each(junctionRecordsForParent, function(junctionRecord) {
                    var childPk = junctionRecord[secondJoin.parentKey];
                    var childRecord = childRecordsByPk[childPk];
                    if (childRecord) {
                      childRecords.push(childRecord);
                    }
                  });

                  // Apply per-parent sort, skip, and limit in JS.
                  if (userSort.length > 0) {
                    _.each(userSort.slice().reverse(), function(sortClause) {
                      var sortKey = _.keys(sortClause)[0];
                      var sortDir = sortClause[sortKey];
                      childRecords = _.sortByOrder(childRecords, [sortKey], [sortDir === 'ASC' ? 'asc' : 'desc']);
                    });
                  }
                  if (userSkip > 0) {
                    childRecords = childRecords.slice(userSkip);
                  }
                  if (userLimit < (Number.MAX_SAFE_INTEGER||9007199254740991)) {
                    childRecords = childRecords.slice(0, userLimit);
                  }

                  childRecordsByParent[parentPk] = childRecords;
                });
          ...
          // Otherwise, if there's one join in the set: no junction table.
            ...
            // Instead of querying per parent record (N+1 problem), collect all unique
            // foreign key values and do a single batched query to the child table.

            // First, handle parent records with undefined FK values (schemaless DBs).
            // (see https://github.com/balderdashy/waterline/issues/1501)
            _.each(populatedParentRecords, function(parentRecord) {
            ...
            // Collect all unique, defined FK values from parent records.
            var allFkValues = _.pluck(populatedParentRecords, singleJoin.parentKey);
            var uniqueFkValues = _.uniq(_.filter(allFkValues, function(val) {
              return !_.isUndefined(val) && !_.isNull(val);
            }));

            // If there are no FK values to look up, skip the query entirely.
            if (uniqueFkValues.length === 0) {
              return nextSetOfJoins(undefined, populatedParentRecords);
            }

            // Build a single batched query using `in` to fetch all child records at once.
            var childTableQuery = _.cloneDeep(baseChildTableQuery);
            var inConjunct = {};
            inConjunct[singleJoin.childKey] = { in: uniqueFkValues };
            childTableQuery.criteria.where.and.push(inConjunct);
            ...
            childTableAdapter.find(childTableDatastoreName, childTableQuery, function(err, childTableResults) {
              ...
              childTableResults = childTableResults || [];

              // Build a lookup dictionary from child key value => child record(s).
              var childRecordsByFk;
              if (singleJoin.collection === true) {
                // For to-many: group by the child's join key (multiple children per FK value).
                childRecordsByFk = {};
                _.each(childTableResults, function(childRecord) {
                  var fkVal = childRecord[singleJoin.childKey];
                  if (!childRecordsByFk[fkVal]) {
                    childRecordsByFk[fkVal] = [];
                  }
                  childRecordsByFk[fkVal].push(childRecord);
                });
              } else {
                // For to-one: index by child key (one child per FK value).
                childRecordsByFk = _.indexBy(childTableResults, singleJoin.childKey);
              }

              // Now assign results back to each parent record.
              _.each(populatedParentRecords, function(parentRecord) {
                var fkVal = parentRecord[singleJoin.parentKey];

                // Skip parents that had undefined FKs (already handled above).
                if (_.isUndefined(fkVal) || _.isNull(fkVal)) {
                  return;
                }

                if (singleJoin.collection === true) {
                  parentRecord[alias] = childRecordsByFk[fkVal] || [];
                } else {
                  var matchedChild = childRecordsByFk[fkVal];
                  parentRecord[singleJoin.parentKey] = matchedChild || null;
                }
              });
